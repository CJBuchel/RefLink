...
pub trait ScheduledService: Send + 'static {
  ...
  /// Returns None by default (no threashold checking)
...
impl ServiceScheduler {
  ...
  pub fn schedule<S: ScheduledService>(service: S, shutdown: &'static ShutdownNotifier) -> JoinHandle<()> {
...
impl SchedulerPool {
  ...
  /// Schedule a periodic service and track its handle
  ...
  /// Spawn a long-running async task and track its handle alongside scheduled services
